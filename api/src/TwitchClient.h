...
class Client {
 public:
  ...
  APIStatus GetOAuthToken(const std::string& code, AuthTokenResult* result);

  APIStatus GetUserByOAuthToken(const std::string& token, UserResult* result);

  APIStatus GetUsersByName(const std::string& name, UsersResult* result);

  APIStatus GetStreamByID(const uint64_t channel_id, StreamsResult* result);

  APIStatus GetChannelByID(const uint64_t channel_id, ChannelsResult* result);

  APIStatus GetVideosByID(const std::string& video_id, VideosResult* result);
 ...
 private:
  ...
  template <typename T>
  APIStatus LoadResultFromURL(const std::string& url, T* result) {
    CurlRequest req(url);
  ...
  template <typename T>
  APIStatus LoadResultFromCurlRequest(const CurlRequest& req, T* result) {
    if (!req.Ok()) {
    ...
    if (req.GetResponseCode() != 200) {
      ...
      if (error.SetData(response.c_str(), response.size()).Ok()) {
        return APIStatus(StatusCode::API_ERROR, error.GetError(),
                         error.GetMessage());
      }
